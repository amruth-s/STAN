  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (this->param_propagate_down_[0] || propagate_down[i]) {
      for (int n = 0; n < this->num_; ++n) {
        ...
        if (propagate_down[i]) {
          this->forward_gpu_gemm(bottom[i]->count() / this->num_, top_diff + n * this->top_dim_, this->blobs_[0]->count(), weight,
              top[i]->count() / this->num_, bottom_diff + n * this->bottom_dim_, NULL, true,
