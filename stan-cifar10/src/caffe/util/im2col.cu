...
__global__ void im2row_gpu_kernel(const int n, const Dtype* data_im,
    ...
    Dtype* data_col) {
      CUDA_KERNEL_LOOP(index, n) {
          ...
          //const int x_index= ((height*width)+blockDim.x-1)/blockDim.x;
	  const int c_col = c_im * kernel_h * kernel_w;
          Dtype* data_col_ptr = data_col;
          //data_col_ptr += ((blockIdx.x/x_index)+(blockDim.x*channels*(blockIdx.x % x_index)) + (threadIdx.x*channels))*kernel_h*kernel_w;
          //data_col_ptr += c_col+((blockDim.x*(blockIdx.x % x_index)) + threadIdx.x)*channels*kernel_h*kernel_w;
